...
import { ArrowDownOnSquareIcon, CheckCircleIcon, TrashIcon } from "@heroicons/react/24/outline";
                                                ...
                                                <tr key={data.id} className="flex justify-between">
                                                    ...
                                                    <td className="flex items-center gap-3 px-3 py-[22px] text-sm text-gray-500 whitespace-nowrap">
                                                        <Tooltip title="Dashboard" placement="bottom">
                                                            <Link to={`/dashboard/survey/${data.id}`}><button className="flex items-center justify-center h-12 gap-2 px-4 font-semibold bg-white border rounded-md hover:bg-gray-100 btn-filled-white bg-brand-white text-light-text-primary border-light-border disabled:opacity-50">
                                                                Dashboard
                                                            </button>
                                                            </Link>
                                                        </Tooltip>

                                                        <Tooltip title="Copy Survey Link" placement="bottom">
                                                            <button
                                                                onClick={() => handleCopy(`${baseUrl}${data.uri}`, data.id)}
                                                                className={clsx(
                                                                    "w-12 h-12 flex items-center justify-center hover:bg-gray-100 hover:border-gray-600 !font-normal rounded-md border",
                                                                    copied && copiedId == data.id ? "bg-gray-100" : "bg-white"
                                                                )}
                                                            >
                                                        ...
                                                        <Tooltip title="Download Survey Data" placement="bottom">
                                                            <button
                                                                onClick={() => handleDownload(data.name, data.participants)}
                                                        ...
                                                        <Tooltip title="Delete Survey" placement="bottom">
                                                            <button onClick={() => handleDeleteSurvey(data.id)} className="bg-white w-12 h-12 flex items-center justify-center hover:bg-gray-100 hover:border-error !font-normal bg-brand-white text-light-text-primary rounded-md border border-light-border">
